...
**Status:** all five subsystems confirmed working (15/15 checks passing as of 2026-06-29).
...
Platform: **Yahboom car** on an **NVIDIA Jetson Orin NX**, **ROS 2 Humble**, Ubuntu 22.04 (L4T R36.4.3).

The thing that trips everyone up is that **two physically different devices share one
USB connector each but expose surprising roles**, so here is the definitive map of
what is plugged in and what it actually does:
...
Two facts from this table drive the whole design of the test scripts:

1. **The "microphone" is the ORBBEC depth sensor.** That is why the audio check
   looks for `orbbec` and not for something called "mic". (See
   [`ROADBLOCKS.md`](ROADBLOCKS.md) for the silent-default-source gotcha.)
2. **The LiDAR (CP2102) and the control board (CH340) both appear as `ttyUSB*`,**
   and the enumeration order is *not* stable across reboots. We therefore never
   trust a bare `ttyUSB0`/`ttyUSB1` number — we use the stable udev symlinks
   `/dev/ydlidar` and `/dev/myserial` and verify which chip is behind each.
...
reaches into each folder and reuses those same classes — nothing is duplicated.

---

## 3. The five per-device checks

Every per-device checker follows the **same three-layer contract**, so once you
...
> will correctly report `busy` — that is expected, not a bug.
...
Two deliberate design choices (both learned the hard way — see `ROADBLOCKS.md`):
capture uses an **external `arecord` process** (a hung in-process PortAudio capture
can wedge Python so hard it ignores Ctrl-C; `arecord` is killable via a subprocess
timeout), and it goes **through pulse** while **pinning `PULSE_SOURCE` to the ORBBEC**
rather than trusting the default source (the default flips to the silent built-in
input across reboots). This check needs **no ROS**.
...
### `test_sensors/test.py` — device & firmware/version inventory

Answers the "identify the available devices" part of the assignment. Two passes:
a universal **USB descriptor scan** of `/sys/bus/usb/devices/*` (vendor + model +
USB release for everything, no dependencies), then **device-specific probes** for a
real firmware version where the device exposes one (the Jetson's L4T version; the
Yahboom board via `Rosmaster_Lib`). Devices with no queryable firmware honestly show
the USB device-release and are marked `usb-descriptor`.
...
```
DEVICE                                VENDOR [usb id]                         VERSION                                          SOURCE
-----------------------------------------------------------------------------------------------------------------------------------------------
MPI7003                               QDtech [0483:5750]                      0200                                             usb-descriptor
ORBBEC Depth Sensor                   Orbbec(R) [2bc5:060f]                   0001                                             usb-descriptor
USB 2.0 Camera                        Sonix Technology Co., Ltd. [2bc5:050f]  0100                                             usb-descriptor
USB Serial                            unknown [1a86:7523]                     8134                                             usb-descriptor
Controller                            unknown [0079:181c]                     1003                                             usb-descriptor
CP2102 USB to UART Bridge Controller  Silicon Labs [10c4:ea60]                0100                                             usb-descriptor
Bluetooth Radio                       Realtek [0bda:c822]                     0000                                             usb-descriptor
Lidar                                 (YDLidar/RPLidar via CP2102)            ydlidar SDK not installed - firmware not probed  ydlidar-sdk
Jetson (host)                         NVIDIA                                  L4T R36.4.3                                      nv_tegra_release
...
See [`ROADBLOCKS.md`](ROADBLOCKS.md) for the full list. Open items as of this
writing:

* **CH340 kernel-driver warning** — the vendor `ch34x` driver logs a benign warning.
  The in-kernel `ch341` driver handles the chip without it. Optional cleanup, does
  not affect operation.
* **LiDAR firmware version not probed** — the YDLidar SDK Python binding isn't
  installed, so `test.py` honestly reports "firmware not probed" rather than faking
  a number. The functional LiDAR check still confirms the device streams data.
